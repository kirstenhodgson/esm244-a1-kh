...
lakes_r_muscosa <- amphibians_clean %>% 
  ...
  mutate(lake = case_when(lake_id != 1 ~ "Lake")) %>% 
  unite(col = lake_id, c(lake, lake_id), sep = " ")

#Need to: total counts of combined adult and subadult by lake, keep only top 5 lakes, fct_reorder to get lakes in ascending or descending order
```
